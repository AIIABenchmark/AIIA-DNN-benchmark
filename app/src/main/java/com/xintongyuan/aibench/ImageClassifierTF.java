...
public abstract class ImageClassifierTF {
  ...
  private static final String TAG = "AIBench";
  ...
  ImageClassifierTF(Activity activity) throws IOException {
    ...
    //imgData = new float[DIM_BATCH_SIZE][getImageSizeX()][getImageSizeY()][DIM_PIXEL_SIZE];
  ...
  public void initlabe(Activity activity){

    testData = new TestData(activity, labelList, "file:///android_asset/val.txt");
  }

  public String runModel() {
    String result ="";
    for (Pair<Integer, Float> pair : topK(5)) {
      result += labelList.get(pair.first) + "\n";
    }
    return result;
  }

  private Pair<Integer, Float>[] topK(int k) {

    int numLabels = getNumLabels();
    final float[] array = new float[numLabels];
    for(int i=0;i<numLabels;i++)
    {
      array[i] = getProbability(i);
     // Log.e(TAG, "result:" + array[i]);
    }

    final boolean[] selected = new boolean[numLabels];
  ...
  public boolean isTop1(String imgName, List<String> result){
    if(testData == null){
      Log.e(TAG, "testData is null");
      return false;
    }
    String imgLabel;
    if(getModelName().equals("vgg16")){
      imgLabel = testData.getImageLabel(imgName);
    }else{
      imgLabel = testData.getImageLabel_1001(imgName);
    }
    Log.i(TAG, "imgName = " + imgName);
    Log.i(TAG, "imgLabel = " + imgLabel);
    if(imgLabel.equals(result.get(0)))
      return true;
    else
      return false;
  }

  public boolean isTop5(String imgName, List<String> result){
    if(testData == null){
      Log.e(TAG, "testData is null");
      return false;
    }
    ...
    for(int i=0; i<RESULTS_TO_SHOW; i++){
      if(imgLabel.equals(result.get(i)))
        return true;
    }
    return false;
  }
  ...
  private MappedByteBuffer loadModelFile(Activity activity) throws IOException {
    ...
    File model_file = activity.getBaseContext().getExternalFilesDir("/models/tflite/");
    String model_path = model_file.getAbsolutePath() + File.separator + getModelPath();
    Log.d(TAG, "model_path:" + model_path);
  ...
  public void convertBitmapToByteBuffer(Bitmap bitmap) {
    ...
    imgData.rewind();
    Bitmap im_resize = Bitmap.createScaledBitmap(bitmap, getImageSizeX(), getImageSizeY(), true);
